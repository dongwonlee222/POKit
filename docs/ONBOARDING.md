...
Codex, 터미널, 임의 도구에서 사용 가능. `./bin/install-codex-plugin`은 Codex plugin 등록과 함께 `~/.local/bin/pokit` CLI wrapper를 설치한다. 새 터미널에서 `pokit`을 찾지 못하면 `~/.local/bin`이 PATH에 있는지 확인한다. 스킬 자동 트리거는 LLM의 자율 판단.
