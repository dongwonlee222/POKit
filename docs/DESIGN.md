...
Phase 2 이후에는 문서 작업의 성격에 따라 페르소나와 검증 구조를 동적으로 선택한다. 목적은 더 많은 에이전트를 쓰는 것이 아니라, 필요한 문맥만 나눠 읽게 해서 토큰 사용량을 줄이고 문서 품질을 높이는 것이다. 병렬 서브에이전트는 runtime이 지원할 때 쓰는 실행 최적화이며, 사용자가 반드시 지켜야 하는 제품 규칙은 아니다.
...
- Phase 2에서는 동적 페르소나와 runtime-supported 병렬 서브에이전트/fallback을 도입한다. 이유: 중요 문서의 품질 검증과 토큰 최적화에 직접적인 가치가 있기 때문이다.
