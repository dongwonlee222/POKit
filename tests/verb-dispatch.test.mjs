...
test("isKnownVerb identifies all 11 verbs", async () => {
  const { isKnownVerb, listKnownVerbs } = await loadModule();
  const verbs = listKnownVerbs();
  assert.equal(verbs.length, 11);
  for (const v of ["start", "brief", "run", "close", "retro", "hotfix", "audit", "guard", "progress", "end", "safety"]) {
