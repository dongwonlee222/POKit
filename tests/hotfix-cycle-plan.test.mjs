...
test("buildHotfixCyclePlans returns create-cycle and move-issue dry-run plans", async () => {
  ...
  assert.equal(plans[0].idempotencyKey, "linear:create_cycle:Hotfix vX.Y.Z:vX.Y.Z");
  assert.equal(plans[1].writes[0].type, "update_issue");
  assert.equal(plans[1].idempotencyKey, "linear:assign_cycle:POKIT-44:<created-hotfix-cycle-id>");
});
...
test("renderHotfixCyclePlanMarkdown shows no-write dry-run and approval boundary", async () => {
  ...
  assert.match(markdown, /Hotfix vX\.Y\.Z/);
  assert.match(markdown, /POKIT-44/);
  assert.match(markdown, /sourceCycle: Cycle N/);
  assert.match(markdown, /targetVersion: vX\.Y\.Z/);
  assert.match(markdown, /resumeCycle: Cycle N\+1/);
